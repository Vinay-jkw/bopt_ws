...
class LoadTransporter(Node):
    def __init__(self, source=None, destination=None, navigator=None):
        ...
        SERVER_HOST = '192.168.68.57'  # Replace with your server's IP address
        SERVER_PORT = 65434
    ...
    def precise_control_to_drop_off_dock(self):
        ...
        result = subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node_with_forktip_check", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(vel), "-p", "initial_steering_angle:=" + str(steering_angle), "-p", "target_distance:=" + str(dist)], capture_output=True, text=True)
        if result.returncode == 0:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.connect(('192.168.68.57', 65434))
    ...
    def task_park(self):
        ...
        # result_to_pose = self.navigate_to_pose(self.navigator, )    
        result_to_poses = self.navigate_through_poses(self.navigator, [[11.81, 21.28, -0.71, 0.69], [7.026, 16.08, -0.71, 0.69]])
