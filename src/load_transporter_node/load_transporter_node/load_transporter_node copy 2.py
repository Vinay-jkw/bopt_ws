...
#         self.pub_fb("Retrieving source and destination data from Database")
#         cancel_status = False
...
#             print(result.stdout)
#             time.sleep(1)
#             # command = ["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node", str(destination_pose[0]), str(destination_pose[1]), str(destination_pose[2]), str(destination_pose[3])]) 
#             # time.sleep(1)
...
# def run_command_with_retry(command, timeout=60, max_retries=3, delay_between_retries=5):
#     """
#     Run a command with a timeout and retries.
...
#         if cancel_status==False:
#             command = ["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(-0.2), "-p", "initial_steering_angle:=" + str(0.0), "-p", "target_distance:=" + str(dist)]
#             result = run_command_with_retry(command)
#             print(result.stdout)
...
#         #     command = ["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(0.2), "-p", "initial_steering_angle:=" + str(0.0), "-p", "target_distance:=" + str(1.2)])
#         # else:
...
#         #             if cancel_status==False:
#         #                 command = ["ros2", "run", "byd_bopt_pallet_picker", "correct_heading", str(fetched_m_offset)])
#         #             if cancel_status==False:
#         #                 command = ["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node", str(pallet_location[0]), str(pallet_location[1]), str(pallet_location[2]), str(pallet_location[3])]) 
#         #             distance_to_midpoint, tensor_value, fetched_m_offset = get_pds_results()
#         #             check_counter += 1
...
#         if cancel_status==False:
#             command = ["ros2", "run", "byd_bopt_pallet_picker", "drive_for_pallet"], capture_output=True, text=True)
#             result = run_command_with_retry(command)
#             print(result.stdout)
...
#     result_pds = command = ["ros2", "run", "pallet_detection", "pallet_detection"], capture_output=True, text=True)
...
#     result_pds = command = ["ros2", "run", "pallet_detection", "pallet_detection_tag"], capture_output=True, text=True)
#     output_string = result_pds.stdout
