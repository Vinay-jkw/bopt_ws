...
class LoadTransporter(Node):
    def __init__(self, source=None, destination=None, navigator=None):
        ...
        G_loaded = nx.read_graphml('/home/byd3/bopt_ws/src/load_transporter_node/load_transporter_node/seabird_graph.graphml')
    ...
    def is_source_value_greater_than_destination(self, source, destination, flow_matrix, flow_matrix_rev):
        ...
        if flow_state == 1:
            station_sequence = {'buffer':0,'p8':1,'p7':2,'p6':3,'p5':4,'p4':5,'p3':6,'p2':7,'p1':8,'pick_1':9,'s1': 10, 's2': 11, 's3': 12,'s4': 13 ,'s5': 14}
        elif flow_state == 2:
            station_sequence = {'s5': 14,'s4': 13, 's3':12,'s2': 11 ,'s1': 10,'pick_1':9,'p1':8,'p2':7,'p3':6,'p4':5,'p5':4,'p6':3,'p7':2,'p8':1,'buffer':0}
    ...
    def go_to_easy_dock_for_pickup_using_flow(self):
        ...
        result_through_poses = self.new_navigate_through_flow_poses(self.navigator, path_coords_with_quaternions, self.rev_source_easy_destination_pose)
        ...
        #     self.pub_fb('navigating easy to dock')
        ...
        #         result_to_pose = self.navigate_to_pose(self.navigator, self.source_easy_destination_pose)
        #     elif flow_state == 1:    
        #         result_to_pose = self.navigate_to_pose(self.navigator, self.source_easy_destination_pose)
        #         # result_to_pose = self.navigate_to_pose(BasicNavigator(), self.source_easy_destination_pose)

        #     else:_with_forktip_check
        #         result_to_pose = self.navigate_to_pose(self.navigator, self.rev_source_easy_destination_pose)
    ...
    def precise_control_to_dock(self): 
        ...
        #     drive_to_point_and_adjust([8.658084,	28.036788,	0.999,	0.025],'down')
        # elif self.source_name == 'p7':   
        #     drive_to_point_and_adjust([7.178084,	27.976788,	0.999,	0.025],'down')
        # if self.source_name in ['p6', 'p7','p8']:
        #     drive_to_point_for_stack([5.778084,	27.866788,	0.999,	0.025],'down') # go to end of stack while looking for pallets
        # elif self.source_name == ['p2','p3', 'p4']:
        # #     drive_to_point_and_adjust([8.68,	25.44006,	0.99,	0.0],'down')
        # # elif self.source_name == 'p3':
        # #     drive_to_point_and_adjust([7.219149,	25.3612,	0.99,	0.01],'down')
        # # elif self.source_name == 'p4':
        #     drive_to_point_for_stack([5.759149,	25.1212,	0.99,	0.01],'down')     
        
        # result = subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node_with_pallet_check", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(0.1), "-p", "initial_steering_angle:=" + str(0.0), "-p", "target_distance:=" + str(1.0)], capture_output=True, text=True)
        ...
        if self.source_name == 'pick1':
            ...
            waypoints.append([22.55,9.0,0.0,1.0])
        if self.source_name == 'pick2':
            waypoints.append(get_current_pose())
            # waypoints.append(self.dock_station_locations['pick2'])
            waypoints.append([22.55,7.74,0.0,1.0])
        if self.source_name == 'pick3':
            waypoints.append(get_current_pose())
            # waypoints.append(self.dock_station_locations['pick2'])
            waypoints.append([22.55,5.97,0.0,1.0])        
        ...
        result = subprocess.run(["ros2", "run", "nmpc_controller", "nmpc_controller_v2_slow_deep_stack",
         "--path_file", "/home/byd3/bopt_ws/src/load_transporter_node/load_transporter_node/constructed_rs_path.pkl"
    ...
    def hault_operation(self,location_name,easy_dock):
        current_formatted_pose = get_current_pose()
        dock_station_location_hault=self.dock_station_locations[location_name]
        tf_pose = get_transformed_pose(dock_station_location_hault)
        if location_name=='nordan1' or location_name=='tgm3':
            ...
            subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node", str(dock_station_location_hault[0]), str(dock_station_location_hault[1]-0.2), str(dock_station_location_hault[2]), str(dock_station_location_hault[3])]) 
        else:
            steering_angle, vel, dist = get_precise_control_key('up', tf_pose)
            result = subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(vel), "-p", "initial_steering_angle:=" + str(steering_angle), "-p", "target_distance:=" + str(dist)], capture_output=True, text=True)
            subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node", str(dock_station_location_hault[0]), str(dock_station_location_hault[1]), str(dock_station_location_hault[2]), str(dock_station_location_hault[3])]) 
        
        station_location_hault=self.station_locations[location_name]
        print(station_location_hault)
        command = ["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node",
                         str(station_location_hault[0]), str(station_location_hault[1]), str(station_location_hault[2]),
                           str(station_location_hault[3])]
        run_command_with_retry(command)

        
        tf_pose = get_transformed_pose(station_location_hault)
        steering_angle, vel, dist = get_precise_control_key('up', tf_pose)
        ...
        if cancel_status==False:
            command = ["ros2", "run", "byd_pose_correction_node_cpp", "pose_correction_node", str(station_location_hault[0]),
                             str(station_location_hault[1]), str(station_location_hault[2]), str(station_location_hault[3])]
            run_command_with_retry(command)
        if location_name == 'tgm4':    
            time.sleep(120)
        else:
            time.sleep(60)
        current_pose = get_current_pose()
        if location_name == 'tgm2':
            dist = np.sqrt((dock_station_location_hault[0] - current_pose[0]) ** 2 + ((dock_station_location_hault[1]-0.2)- current_pose[1]) ** 2)
        elif location_name=='nordan1':
            dist = np.sqrt((dock_station_location_hault[0] - current_pose[0]) ** 2 + ((dock_station_location_hault[1]-0.3)- current_pose[1]) ** 2)
        elif location_name=='tgm3':
            dist = np.sqrt((dock_station_location_hault[0] - current_pose[0]) ** 2 + ((dock_station_location_hault[1]-0.3)- current_pose[1]) ** 2)    

        else:    
            dist = np.sqrt((dock_station_location_hault[0] - current_pose[0]) ** 2 + (dock_station_location_hault[1]- current_pose[1]) ** 2)
        ...
        if location_name == 'tgm4':
            self.navigate_through_poses(self.navigator, [[58.0, 13.9, self.easy_station_locations["tgm4"][2], self.easy_station_locations["tgm4"][3]]])

        elif location_name == 'tgm3':
            # drive_to_point_and_adjust(self.easy_station_locations["tgm3"],'down')
            self.navigate_through_poses(self.navigator, [[50.0, 13.9, easy_dock[2], easy_dock[3]]])

        elif location_name=='tgm2':  
            self.navigate_through_poses(self.navigator, [[40.0, 13.9, easy_dock[2], easy_dock[3]]])  
            # drive_to_point_and_adjust(easy_dock, 'down')
        elif location_name=='nordan1':  
            self.navigate_through_poses(self.navigator, [[27.0, 13.9, easy_dock[2], easy_dock[3]]])

    def hault_operation_forward(self,location_name,easy_dock):
        current_formatted_pose = get_current_pose()
        dock_station_location_hault=self.dock_station_locations[location_name]
        tf_pose = get_transformed_pose(dock_station_location_hault)
        steering_angle, vel, dist = get_precise_control_key('up', tf_pose)
        ...
        if cancel_status==False:
            subprocess.run(["ros2", "run", "byd_pose_correction_node_cpp", "precise_control_node", "my_node", "--ros-args", "-p", "initial_velocity:=" + str(-0.2), "-p", "initial_steering_angle:=" + str(0.0), "-p", "target_distance:=" + str(dist)])
        if location_name == 'tgm3':
            # drive_to_point_and_adjust(self.easy_station_locations["tgm3"],'down')
            self.navigate_through_poses(self.navigator, [[54.5, 14.0, easy_dock[2], easy_dock[3]]])

        elif location_name=='tgm2':  
            self.navigate_through_poses(self.navigator, [[45.0, 14.0, easy_dock[2], easy_dock[3]]])  
            # drive_to_point_and_adjust(easy_dock, 'down')

        elif location_name=='nordan1':  
            self.navigate_through_poses(self.navigator, [[31.5, 14.0, easy_dock[2], easy_dock[3]]])              
    ...
    def go_to_drop_off_easy_dock_using_flow(self):
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.rev_easy_station_location["nordan1"]
        ...
        run_command_with_retry(command)
        self.hault_operation_forward("nordan1",final_point)
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.rev_easy_station_location["tgm2"]
        ...
        run_command_with_retry(command)
        self.hault_operation_forward("tgm2",final_point)
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.rev_easy_station_location["tgm3"]
        ...
        run_command_with_retry(command)
        self.hault_operation_forward("tgm3",final_point)
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.rev_easy_station_location["tgm4"]
        ...
        self.hault_operation("tgm4",final_point)

        # self.navigate_through_poses(self.navigator, [[32.1456, 3.5684, 0.999, 0.018]])
        # self.navigate_through_poses(self.navigator, [[32.1456, 3.5684, 0.999, 0.018]])
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.easy_station_locations["tgm3"]
        ...
        self.hault_operation("tgm3",final_point)
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.easy_station_locations["tgm2"]
        ...
        self.hault_operation("tgm2",final_point)
        ...
        if self.destination_name == 'buffer':
            ...
            final_point = self.easy_station_locations["nordan1"]
        ...
        self.hault_operation("nordan1",final_point)
    ...
    def precise_control_to_drop_off_dock(self):
        ...
        self.send_data(b"8")
        line_empty=False
        drop_no=int(self.destination_name[-1])
        while line_empty==False:
        # try:
        #     with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        #         s.connect(('192.168.68.57', 65434))
        #         s.sendall(b"8")
        ...
        #dock_location
            ...
            if res=='True':
                drop_no=drop_no+1
                self.destination_destination_pose=self.dock_station_locations['drop'+str(drop_no)]
                self.rev_easy_location=self.rev_easy_station_location['drop'+str(drop_no)]
                # self.rev_easy_location[0]=self.rev_easy_location[0]+0.15
                # drive_to_point_and_adjust(self.rev_easy_location,'up')
                self.points_to_navigate=self.rev_easy_station_location['drop'+str(drop_no)]
                self.navigate_through_poses(self.navigator, [[self.points_to_navigate[0]+0.15, self.points_to_navigate[1], self.points_to_navigate[2], self.points_to_navigate[3]]]) 
                self.pallet_manager_drop.dock_location=self.destination_destination_pose
                # drop_no=drop_no+1
            else:
                line_empty=True    
            fun_res=True
        ...
        #     drive_to_point_and_adjust([2.53487,	3.074365,	-0.71341,	0.700739],'up')
    ...
    def go_for_pallet_drop(self):
        ...
        if res=='True':
            ...
            if self.destination_name == 'drop1':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.35,8.048,0.9993,0.03702])
            elif self.destination_name == 'drop2':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,10.9,0.9993,0.03702])
            elif self.destination_name == 'drop3':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,self.destination_destination_pose[1],0.9993,0.03702])
            elif self.destination_name == 'drop4':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,self.destination_destination_pose[1],0.9993,0.03702])
            elif self.destination_name == 'drop5':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.35,self.destination_destination_pose[1],0.9993,0.03702])
            elif self.destination_name == 'drop6':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,self.destination_destination_pose[1],0.9993,0.03702])
            elif self.destination_name == 'drop7':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,self.destination_destination_pose[1],0.9993,0.03702])
            elif self.destination_name == 'drop8':
                waypoints.append(get_current_pose())
                # waypoints.append(self.dock_station_locations['pick2'])
                waypoints.append([1.0,self.destination_destination_pose[1],0.9993,0.03702])                
    ...
    def pose_correct_on_flow_after_drop(self):
        ...
        if self.destination_name=="drop3" or self.destination_name=="drop4":
            ...
            drive_to_point_and_adjust(self.rev_destination_easy_destination_pose, 'down')

        self.navigate_through_poses(self.navigator, [[9.498, 3.703, 0.6645, 0.7472]])    
        # if self.destination_name == 'buffer':
