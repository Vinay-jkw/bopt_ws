...
class LoadTransporter(Node):
    ...
    def go_to_easy_dock_for_pickup_using_flow(self):
        ...
        # flow_matrix = self.flow_matrix
        flow_state = self.get_flow_state(flow_matrix, self.rev_flow_matrix)

        self.pub_fb("Navigating through waypoints")
        print("flow_statee  ---====",flow_state)
        try:
            # find path to station 
            if (self.source_name == 'pick_1' or self.source_name== 'p5' or self.source_name=='p6' or self.source_name=='p7' or self.source_name=='p8' or self.source_name=='p1' or self.source_name=='p2' or self.source_name=='p3' or self.source_name=='p4') and flow_no == 2:
                print("fingding_through_rev_flow")
                if flow_state == 1:
                    path_with_names, path_with_coords = self.find_path_from_current_location(current_formatted_pose, self.source_name,
                                                                                            flow_matrix, self.easy_station_locations)
                else:
                    path_with_names, path_with_coords = self.find_path_from_current_location(current_formatted_pose, self.source_name,
                                                                                            rev_flow_matrix, self.rev_easy_station_location)

            elif (self.source_name == 'pick_1' or self.source_name== 'p5' or self.source_name=='p6' or self.source_name=='p7' or self.source_name=='p8' or self.source_name=='p1' or self.source_name=='p2' or self.source_name=='p3' or self.source_name=='p4') and flow_no == 1:
                if flow_state == 1:
                    path_with_names, path_with_coords = self.find_path_from_current_location(current_formatted_pose, self.source_name,
                                                                                            flow_matrix, self.rev_easy_station_location)
                else:
                    path_with_names, path_with_coords = self.find_path_from_current_location(current_formatted_pose, self.source_name,
                                                                                        rev_flow_matrix, self.easy_station_locations)                                                                            
            ...
            if len(path_with_coords) > 2:#check
                if self.source_name == 'pick_1' or self.source_name== 'p5' or self.source_name=='p6' or self.source_name=='p7' or self.source_name=='p8' or self.source_name=='p1' or self.source_name=='p2' or self.source_name=='p3' or self.source_name=='p4':
                    result_through_poses = self.new_navigate_through_flow_poses(self.navigator, path_with_coords, self.source_easy_destination_pose)
                    
                elif flow_state == 1:
                    result_through_poses = self.new_navigate_through_flow_poses(self.navigator, path_with_coords, self.source_easy_destination_pose)
                else:
                    result_through_poses = self.new_navigate_through_flow_poses(self.navigator, path_with_coords, self.rev_source_easy_destination_pose)
    ...
    def precise_control_to_dock(self): 
        ...
        if self.source_name == 'p6':
            ...
            drive_to_point_and_adjust([5.778084,	27.866788,	0.999,	0.025],'down')
            ...
            drive_to_point_and_adjust([5.759149,	25.1212,	0.99,	0.01],'down')            
    ...
    def go_for_pallet_pickup(self):
        ...
        if sta == 'False':
            self.pallet_picked=False
            if self.source_name=='p6' or self.source_name=='p7' or self.source_name=='p8' or self.source_name=='p2' or self.source_name=='p3' or self.source_name=='p4':
    ...
    def pose_correct_on_flow_after_lift(self):
        ...
        flow_break=self.is_source_value_greater_than_destination(self.source_name,self.destination_name,self.flow_matrix,self.rev_flow_matrix)
        flow_state = self.get_flow_state(self.flow_matrix, self.rev_flow_matrix)
        # if flow_state == 2 and flow_break:
        #     print("pose_correct_to_rev_flow")
        #     self.pallet_manager_lift_rev.pose_correct_on_flow()
        # elif flow_state == 1 and flow_break:
        #     print("pose_correct_to_flow")
        #     self.pallet_manager_lift.pose_correct_on_flow()   
        # if flow_state == 2:
        #     print("pose_correct_to_rev_flow")
        #     self.pallet_manager_lift_rev.pose_correct_on_flow()
        # else:
        ...
        if self.source_name == "T1" or self.source_name == "T2" and cancel_status == False:
            print('going to flow')
            ...
            nearest_flow_point_to_station,_ = self.find_nearest_point_from_current_location_to_flow(current_formatted_pose, self.flow_matrix)
            result_to_pose = self.navigate_to_pose(self.navigator, self.flow_matrix[nearest_flow_point_to_station-1])
            result_to_pose = self.navigate_to_pose(self.navigator, self.flow_matrix[nearest_flow_point_to_station+1])
    ...
    def go_to_drop_off_easy_dock_using_flow(self):
        ...
        self.state=robot_state
        if self.destination_name == 'T1':
            self.navigate_to_pose(self.navigator, self.flow_matrix[1])
        if flow_state == 1:    
    ...
    def go_for_pallet_drop(self):
        ...
        if res=='True':
            ...
            self.pallet_manager_drop.run_drop_pallet()
    ...
    def pose_correct_on_flow_after_drop(self):
        ...
        self.thread2.start()
        # if stations = t1, t2 navigate to nearest point in flow
        if self.destination_name == "T1" or self.destination_name == "T2" and cancel_status == False:
            print('going to flow')
            #Find current Location
            current_formatted_pose = get_current_pose()
            robot_state = 'Going to Flow'
    ...
    def new_navigate_through_flow_poses(self, navigator, path_with_coords, easy_destination_pose):
        ...
        for index, goal_pose in enumerate(goal_poses):
            ...
            while index < len(goal_poses) - 1 and self.dist_checker(goal_pose, tolerance=4.5):
                if time.time() - start_time > timeout_duration:
                    if goal_attempts[pose_key] == 0:
...
def main(args=None):
    ...
    transporter = LoadTransporter(source, destination)
        ...
        if state >= start_point:
            ...
            if stat == False:
                if state==3:
                    ...
                    subprocess.run(["ros2 run load_transporter_node load_transporter_node_v4 "+str(des)+" "+'buffer'+" 5","arguments"],shell=True)
                    break
    ...
    if des!="buffer":
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if error_found == False:
                    if error_state==3:
                        s.sendall(b"Pallet_Not_Present")
                    elif error_state==9:
                        s.sendall(b"task_incomplete_pallet_not_dropped")    
                    else:
                        s.sendall(b"Task ended")    
                else:
                    s.sendall(b"Task ended")
                data = s.recv(1024).decode()
