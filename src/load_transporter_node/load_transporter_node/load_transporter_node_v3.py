...
class LoadTransporter(Node):
    ...
    def go_for_pallet_pickup(self):
        ...
        self.pallet_manager_lift.run_drive_for_pallet()
    ...
    def reorient_to_flow(self):
        ...
        self.current_state_thread = Thread(target=self.bopt_current_state_publisher, args=(str(self.current_state_of_task),)) 
        self.current_state_thread.start()
        if self.source_name == "T1" or self.source_name == "T2":
            pass
        else:
    ...
    def pose_correct_on_flow_after_lift(self):
        ...
        if self.source_name == "T1" or self.source_name == "T2" and cancel_status == False:
            ...
            nearest_flow_point_to_station = self.find_nearest_point_from_current_location_to_flow(current_formatted_pose, self.flow_matrix)
    ...
    def go_to_drop_off_easy_dock_using_flow(self):
        ...
        try:
            #find path to station 
            path_with_names, path_with_coords = self.find_path_from_current_location(current_formatted_pose, self.destination_name, flow_matrix, self.easy_station_locations)
    ...
    def reorient_to_flow_after_drop_off(self):
        ...
        if self.destination_name == "T1" or self.destination_name == "T2":
            pass
        else:
        ...
        try: 
            ...
            if abs(fetched_m_offset) > 0.05:
                ...
                while check_counter < 3:
                    if cancel_status==False:
                        subprocess.run(["ros2", "run", "byd_bopt_pallet_picker", "correct_heading",
