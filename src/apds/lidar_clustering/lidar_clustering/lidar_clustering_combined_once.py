...
def save_points_to_json(before: np.ndarray, after: np.ndarray, lidar):
    data = {
        "points_before": before.tolist(),
        "points_after": after.tolist()
    }
...
class LidarClusteringNodeRight(Node):
    ...
    def lidar_callback(self, msg: LaserScan):
        ...
        # save_points_to_json(points, transformed_points, "left")
        # Process clusters
    ...
    def pro_lidar_callback(self, points: np.ndarray, rois: List[Tuple[float, float, float, float]], window_name: str):
        ...
        cluster_centers_all_rois = self.get_lowest_y(points_all_rois)
    ...
    def get_lowest_y(self, points_all_rois: dict):
        """
        For each ROI, return the point with the lowest Y value.
        """
        results = []

        for roi_index, (roi_name, roi_points) in enumerate(points_all_rois.items(), start=1):
            if roi_points.shape[0] > 0:
                lowest_point = roi_points[np.argmin(roi_points[:, 1])]
                results.append((roi_index, lowest_point))
            else:
                results.append((roi_index, None))
...
class LidarClusteringNodeLeft(Node):
    ...
    def lidar_callback(self, msg: LaserScan):
        ...
        rois = calculate_rois(0.80, 1.40)
        points = self.calculate_points(msg)
        self.roi_cluster_mapping = {i + 1: "No Data" for i in range(len(rois))}

        # Transform points into base_link frame
        transformed_points = apply_transformation(points, self.translation, self.rotation)
        # save_points_to_json(points, transformed_points, "right")
        # Process clusters
...
class TwinLidarNode(Node):
    ...
    def parse_cluster_data(self, data: str) -> List[Optional[Tuple[float, float]]]:
        ...
        """
        clusters = [None] * 2  # Assuming 9 ROIs
        lines = data.split('\n')
        for line in lines:
            line = line.strip()
