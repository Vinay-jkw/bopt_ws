...
class LidarClusteringNodeRight(Node):
    ...
    Subscribes to /Lidar_RFT and publishes cluster centers to the 'map1' topic.
    ...
    def lidar_callback(self, msg: LaserScan):
        rois = calculate_rois(1.22, 1.22)  # or adjust pallet size
...
class LidarClusteringNodeLeft(Node):
    ...
    Subscribes to /Lidar_LFT and publishes cluster centers to the 'map2' topic.
    Also publishes ROI visualization markers to 'roi_marker'.
    """
    def __init__(self):
...
class TwinLidarNode(Node):
    ...
    Listens for 3 seconds on each topic, accumulates cluster data, averages them,
    and finally publishes combined results (pallet presence, middle offset, angle offset)
    on 'pallet_detection_results'. Then stops further processing.
    """
    def __init__(self):
    ...
    def callback_1(self, msg: String):
        ...
        if self.processing_done:
            return  # Stop further processing once done

        cluster_data = self.parse_cluster_data(msg.data)

        if self.lidar1_start_time is None:
            self.lidar1_start_time = time.time()  # Start the timer for LIDAR1

        # Aggregate data
        if self.lidar1_buffer is None:
            # Initialize buffer with empty lists, one per ROI
    ...
    def callback_2(self, msg: String):
        ...
        if self.lidar2_start_time is None:
            self.lidar2_start_time = time.time()  # Start the timer for LIDAR2
    ...
    def aggregate_clusters(self, 
                           buffer: List[List[Optional[Tuple[float, float]]]], 
                           data: List[Optional[Tuple[float, float]]]):
        """
        Append valid (non-None) cluster data to the buffer for each ROI index.
        buffer is structured like: buffer[roi_index] = [ (x1,y1), (x2,y2), ... ]
        """
        for i, cluster in enumerate(data):
            if cluster is not None:
                buffer[i].append(cluster)

    def calculate_midpoint_clusters(self, buffer: List[List[Tuple[float, float]]]) -> List[Optional[Tuple[float, float]]]:
        """
        For each ROI, calculate the mean (x, y) across all accumulated valid points.
        Returns a list of the same length as # of ROIs: either (mean_x, mean_y) or None.
        """
        midpoint_clusters = []
        for cluster_points in buffer:
            if len(cluster_points) > 0:
                ...
                midpoint_clusters.append((round(mean_x, 2), round(mean_y, 2)))
            else:
                midpoint_clusters.append(None)
        return midpoint_clusters

    def check_and_process(self):
        """If both LIDARs have completed 3s accumulation, process combined data."""
    ...
    def process_combined_data(self):
        ...
        Combine the data from both LIDARs:
          - If both sides have a valid cluster for ROI, take the midpoint
          - If only one side has it, use that
          - If none have it, None
        Then publish a final message with:
          - Pallet presence (>=6 clusters)
          - Middle offset (ROI #2 is the middle ROI, example usage)
          - Angle offset (based on ROI #1 and ROI #3)
        """
        combined_data = []
        for c1, c2 in zip(self.lidar1_data, self.lidar2_data):
            if c1 is not None and c2 is not None:
                combined_data.append((round((c1[0] + c2[0]) / 2, 2),
                                      round((c1[1] + c2[1]) / 2, 2)))
            elif c1 is not None:
    ...
    def publish_results(self, combined_data: List[Optional[Tuple[float, float]]]):
        ...
        if len(combined_data) >= 3 and combined_data[0] is not None and combined_data[2] is not None:
