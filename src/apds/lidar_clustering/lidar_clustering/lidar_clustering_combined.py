...
#     rois = [
#         # Bottom row (ROI 1, 2, 3)
#         (-b2 - 0.05 - roi_width / 2, b2 + 0.05 + roi_width / 2, lidar_offset-0.05, lidar_offset + roi_height+l),
#     ]
#     return rois
...
class TwinLidarNode(Node):
    ...
    Periodically (e.g., every 3 seconds) accumulates cluster data,
    averages them, and publishes combined results (pallet presence, middle offset, angle offset)
    on 'pallet_detection_results'.
    """
    def __init__(self):
        ...
        self.lidar1_buffer = [[] for _ in range(9)]  # Assuming 9 ROIs
        self.lidar2_buffer = [[] for _ in range(9)]  # Assuming 9 ROIs

        # Create a timer to process data every 3 seconds
        self.processing_timer = self.create_timer(3.0, self.process_combined_data)

        self.get_logger().info("Twin Lidar Node Initialized for Continuous Processing")
    ...
    def process_combined_data(self):
        """Process accumulated data from both LIDARs and publish results."""
        # Calculate midpoints for both LIDARs
        lidar1_data = self.calculate_midpoint_clusters(self.lidar1_buffer)
        lidar2_data = self.calculate_midpoint_clusters(self.lidar2_buffer)

        # Combine the data from both LIDARs
        combined_data = []
        for c1, c2 in zip(lidar1_data, lidar2_data):
        ...
        # Reset the buffers for the next cycle
        self.lidar1_buffer = [[] for _ in range(9)]
        self.lidar2_buffer = [[] for _ in range(9)]
