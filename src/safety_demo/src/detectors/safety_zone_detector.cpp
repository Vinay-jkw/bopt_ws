...
SafetyZoneDetector::defineFieldPolygon(const std::string &shape_type,
...
bool SafetyZoneDetector::isPointInPolygon(
  ...
  for (size_t i = 0, j = n - 1; i < n; j = i++) {
     ...
     */
    const double cross =
        (yj - yi) * (point.first - xi) - (xj - xi) * (point.second - yi);

    if (std::abs(cross) < 1e-9 && point.first >= std::min(xi, xj) &&
        point.first <= std::max(xi, xj) && point.second >= std::min(yi, yj) &&
     ...
     */
    const bool crosses = ((yi > point.second) != (yj > point.second));

    if (crosses) {
      const double x_intersection =
          (xj - xi) * (point.second - yi) / (yj - yi) + xi;
...
std::pair<double, double> SafetyZoneDetector::polarToCartesian(double range,
                                                               double angle) {
