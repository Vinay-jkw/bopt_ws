...
SLOW_DOWN_DISTANCE = 2.3 # Distance within which the robot starts to slow down
...
class RobotClient(Node):
    def __init__(self):
        ...
        self.declare_parameter('path_file', '/home/fbots/bopt_v2_ws/src/load_transporter_node/load_transporter_node/constructed_rs_path.pkl')
        ...
        def param_changes(params):
            ...
            for param in params:
                if param.name == 'should_halt':
                    ...
                    if not param.value: _MAX_VEL = 1.42
    ...
    def conflict_action_callback(self, msg):
        ...
        print(f"Conflict MSG:", msg, file=open('nmpc.log', 'w'))
    ...
    def get_target_point(self, robot_x, robot_y):
        ...
        self.get_logger().info(f"lookahead is: {lookahead_distance}")        
    ...
    def calculate_curvature(self, lookahead):
        ...
        self.get_logger().info("Calculating curvature for lookahead: {lookahead}")
    ...
    def follow_path(self):
        ...
        self.get_logger().info(f"MAX_VELOCITY:=============== {MAX_VELOCITY}")
        ...
        self.get_logger().info(f"final_point: {self.final_point}, robot_state: {robot_x, robot_y}")
        self.get_logger().info(f'DTG: {distance_to_goal}')
        if distance_to_goal <= self.goal_tolerance:
            self.goal_reached = True
            self.send_stop_command(0.0, 0.0)  # Stop the robot
            self.get_logger().info('Goal reached DTG, shutting down...')
            # self.destroy_node()
            # rclpy.shutdown()
            # sys.exit(0)
            return
        distance_to_lp = math.hypot(self.path_last_point[0] - robot_x, self.path_last_point[1] - robot_y)
        self.get_logger().info(f"path_last_point: {self.final_point}, robot_state: {robot_x, robot_y}")
        self.get_logger().info(f'DTLP: {distance_to_lp}')
        if distance_to_lp <= self.goal_tolerance:
            self.goal_reached = True
            self.send_stop_command(0.0, 0.0)  # Stop the robot
            self.get_logger().info('Goal reached DTLP, shutting down...')
            # self.destroy_node()
            # rclpy.shutdown()
            # sys.exit(0)
            return
        ...
        self.get_logger().info("velocity after s-curve {self.current_velocity:.2f}")
        ...
        # To reduce wave, applying curvature filter for velocity control
        k = 4
        curv = abs(self.calculate_curvature(1))
        curv_threshold = 0.02
        min_motion_velocity = 0.15
        min_motion_velocity_safe1 = 0.05
        if curv < curv_threshold:
            safe_velocity_2 = _MAX_VEL
        else:
            safe_velocity_2 = max(min_motion_velocity,  abs(_MAX_VEL * np.exp(-k * (curv - curv_threshold))))
            # safe_velocity_2 = MAX_VELOCITY / ( 1 + k * abs(self.calculate_curvature(1.2)))

        safe_velocity_1 = max(min_motion_velocity_safe1, abs(safe_velocity_1))
        safe_velocity = min(abs(safe_velocity_1), abs(safe_velocity_2))

        self.get_logger().info(f"safe_velocity_2: {safe_velocity_2:.2f}, safe_velocity_1: {safe_velocity_1:.2f}")
        self.get_logger().info(f"safe_velocity: {safe_velocity:.2f}")
        ...
        self.get_logger().info(f"Velocity: {self.current_velocity:.2f}, Wheel_velocity:{self.wheel_velocity:.2f}")
        self.get_logger().info(f"Curvature: {self.calculate_curvature(1.2):.5f}")
    ...
    def calculate_safe_velocity(self, cv, steering_angle):
        ...
        safe_velocity = 1.0 * cv * (1 / (1 + np.exp(-(0.93 * np.sqrt(mu * (g / abs(curvature)))))))
