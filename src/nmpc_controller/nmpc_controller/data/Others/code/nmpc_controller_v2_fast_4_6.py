...
class RobotClient(Node):
    def __init__(self, params):
        ...
        with open('/home/fbots/bopt_v2_ws/src/nmpc_controller/nmpc_controller/mpc_lookup_table_2.5.pkl', 'rb') as f:
    ...
    def generate_temporary_path1(self, original_path, current_node, deviation_distance=2.8, nodes_to_split=8):
        ...
        # Precompute 45° rotation factors

        angle_45 = math.pi / 4

        cos45 = math.cos(angle_45)

        sin45 = math.sin(angle_45)
        ...
        for i, node in enumerate(next_nodes):
            ...
            # Check if this is a T-junction (degree == 3)

            deg = self.G_loaded.degree[node]
            print("Degree of node", node, "is", deg)
            if deg == 6:
                if (not dy and dx) or (not dx and dy):
                    continue
                # Compute forward unit vector
                ux = dx / length

                uy = dy / length

                # Rotate by +45° for left-forward offset

                rx = ux * cos45 - uy * sin45

                ry = ux * sin45 + uy * cos45

                temp_x = node_position[0] + deviation_distance * rx

                temp_y = node_position[1] + deviation_distance * ry

            else:

                # Standard 90° left perpendicular offset
        ...
        return [original_path[current_index]] + temporary_nodes + original_path[current_index + nodes_to_split+1:-1]

    

    # def generate_temporary_path(self, original_path, current_node, deviation_distance=0.7, node_to_split=8):
    ...
    #     # Get the next few nodes
    #     next_nodes = original_path[current_index + 1:current_index + node_to_split]
    ...
    #     return [original_path[current_index]] + temporary_nodes + original_path[current_index + node_to_split+1:-1]
    ...
    def generate_temporary_path(
        ...
        # ------------------------------------------------------------------
        final_path = list(reversed(pruned_path)) if reverse_output else pruned_path
        return final_path
    ...
    #     distance_threshold = 0.7# meters (can be adjusted based on your needs)
    ...
    def update_remaining_path_in_nodes(self,robot_x,robot_y):
        ...
        if reached_index is not None:
            ...
            # Advance the external index that tracks our progress, if you keep one
            self.path_index_in_nodes += reached_index + 1
    
            # Slice away all nodes up to & incl. the reached node
            self.path_in_nodes = self.path_in_nodes[reached_index + 1 :]
    ...
    def handle_local_path(self,local_path):
        ...
        # print(f"Path with quaternions: {path_with_quaternions}")
        # path_with_quaternions = self.filter_points_within_radius(local_path[0], path_with_quaternions, 1.0)
        # path_with_quaternions = self.filter_points_within_radius(local_path[-1], path_with_quaternions, 1.0)
