...
class RobotClient(Node):
    ...
    def follow_path(self):
        ...
        self.rem_dist = distance_to_goal
        ...
        if distance_to_goal <= SLOW_DOWN_DISTANCE:
            target_velocity = (distance_to_goal / (SLOW_DOWN_DISTANCE)) * PICKING_VELOCITY
            if velocity < 0:
    ...
    def calculate_safe_velocity(self, steering_angle):
        ...
        safe_velocity = 0.15 * np.sqrt(mu * (g / curvature))
    ...
    def pallet_detection_callback(self, msg):
        print(self.rem_dist, '=====')
        if self.rem_dist != None:
            if self.rem_dist > 0.5:
                self.pallet_detected = msg.data
        self.get_logger().info(str(msg))
    
    def odot_callback(self, msg):
        # sensor_data = msg.data.split(' ')[1]
        # back_sensor_byte = sensor_data[0] #back sensors
        # front_sensor_byte = sensor_data[-1] #forktip sensors
        # if front_sensor_byte in ["0"]:
