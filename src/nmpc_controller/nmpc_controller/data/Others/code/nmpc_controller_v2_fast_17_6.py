...
class RobotClient(Node):
    ...
    def generate_local_path(self):
        ...
        if current_node:
            ...
            path_with_quaternions = self.filter_points_within_radius(self.dock_location_dict[self.destination][:2], path_with_quaternions, 2.0)
    ...
    def send_robot_to_buffer_local_path(self, buffer_node):
        ...
        final_dest = self.path_in_nodes[-1]

        # 3) compute a valid graph path: current → buffer → final
        first_leg = self.find_valid_path_using_nearest_nodes(source=current_node, target=buffer_node)
        second_leg = self.find_valid_path_using_nearest_nodes(source=buffer_node, target=final_dest)
        new_node_list = first_leg + second_leg

        # 4) publish “we’re on a local path now”
        self.publish_local_path_active(True)

        # 5) convert node list → (x,y,qz,qw)
        path_with_quats = self.handle_local_path(new_node_list)

        # 6) optionally append final docking quaternion for final_dest
        path_with_quats.append(self.dock_station_end_line_dict[final_dest])

        # 7) build a spline
        spline_path = self.generate_spline_path(
            self.get_node_position(current_node),
            path_with_quats,
            scale=1.0,
            runway_length=0.0,
            spacing=0.01
        ...
        # 8) overwrite state
        self.path = spline_path
        self.path_in_nodes = new_node_list
        # Optionally save to files if needed:
        with open('/home/fbots/bopt_v2_ws/src/workflow_node/workflow_node/constructed_rs_path_in_nodes.pkl', 'wb') as file:
            pickle.dump(self.path_in_nodes, file)
        with open('/home/fbots/bopt_v2_ws/src/workflow_node/workflow_node/constructed_rs_path.pkl', 'wb') as file:
    ...
    def generate_temporary_path(
        ...
        # ------------------------------------------------------------------
        final_path = (list(reversed(pruned_path)) + [original_path[-1]]) if reverse_output else (pruned_path + [original_path[-1]])
        return final_path
    ...
    def handle_conflict(self):
        if self.conflict_action == "halt":
            ...
            self.conflict_action = None
            return
        elif self.conflict_action == "buffer":
            self.path_index = 0
            self.path_received = False
            self.goal_reached = False
            self.path_in_nodes_received = False
            self.send_robot_to_buffer_local_path(self.buffer_id)
            self.conflict_action = None
            return
