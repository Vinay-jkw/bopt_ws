...
class RobotClient(Node):
    ...
    def generate_local_path(self):
        print("Conflict detected: Splitting path and generating a local path...")
        ...
        if current_node:
            ...
            print("New local path:", local_path)
            ...
            end_easy_dock = self.find_left_or_right_dock(self.left_easy_dock_dict[self.destination], self.right_easy_dock_dict[self.destination], self.dock_location_dict[self.destination], path_with_quaternions[-1], is_last_pose=True)
    ...
    def calculate_curvature_finite_diff(self, path):
        ...
        try:
            ...
            self.get_logger().error(f"Error in calculate_curvature_finite_diff: {e}")
            return 0
    ...
    def get_target_point(self, robot_x, robot_y):
        ...
        self.get_logger().debug(f"Calculated curvature: {curvature:.5f}, Adjusted lookahead distance: {lookahead_distance:.2f}")
    ...
    def generate_temporary_path(self, original_path, current_node, deviation_distance=0.7):
        ...
        next_nodes = original_path[current_index + 1:current_index + 6]
        ...
        return [original_path[current_index]] + temporary_nodes + original_path[current_index + 6:-1]
    

    def update_remaining_path_in_nodes(self,robot_x,robot_y):
        """
        ...
        distance_threshold = 1.0# meters (can be adjusted based on your needs)
        ...
        # Check distance to each node in path_in_nodes.
        while self.path_in_nodes:
            next_node = self.path_in_nodes[0]
            if isinstance(next_node, tuple) and str(next_node[0]).startswith("temp"):
                node_x, node_y = next_node[1][0], next_node[1][1]
            else:
                node_x, node_y = self.get_node_position(next_node)
            

            # Calculate distance to the node
            distance_to_node = math.sqrt((robot_x-node_x)**2 + (robot_y-node_y)**2)
            # print(distance_to_node, "DTN++++++++++++++++++")
            self.get_logger().info(f"Node {next_node} reached. Remaining path: {self.path_in_nodes}")


            # If the robot is close enough to the node, remove it from the path
            if distance_to_node <= distance_threshold:
                self.path_index_in_nodes+=1
                self.path_in_nodes.pop(0)  # Remove the first node (i.e., the reached node)
            else:
                break  # Stop checking once we've found a node that's too far

        self.publish_remaining_path()


    def publish_remaining_path(self):
        remaining_path_str = ",".join(str(item) for item in self.path_in_nodes)
    ...
    def calculate_curvature(self, lookahead):
        ...
        :return:
        """
        self.get_logger().debug("Calculating curvature for lookahead: {lookahead}")
        segmented_path = self.path[self.path_index:self.path_index + int(lookahead * 100)]

        if len(segmented_path) < 3:
            self.get_logger().error("Not enough points in segmented_path for curvature calculation")
            return 0.01
    ...
    def follow_path(self):
        ...
        self.get_logger().info("FOLLOW_PATH START")
        ...
        state = self.current_pose
        if state is None:
            self.get_logger().error(f"state is none {self.goal_reached}")
            return
        ...
        self.update_remaining_path_in_nodes(robot_x,robot_y)
        
        if self.local_path_active:
            self.max_velocity=0.3
        else:
            self.max_velocity=self.max_velocity
        # self.get_logger().info(f"Nearest key: {self.find_nearest_key(ttp)}")      #EDITED
        # self.get_logger().info(f"Lookup table value: {self.lookup_table.get(self.find_nearest_key(ttp))}")
        # s, v, d = self.lookup_table.get(self.find_nearest_key(ttp))
        
        
        try:
            # ONLY risky math / lookup
            key = self.find_nearest_key(ttp)
            val = self.lookup_table.get(key)

            if val is None:
                raise ValueError(f"No lookup for key {key}")
        ...
        self.get_logger().info(f"Without change ( from lookup )::: Steering (lookup): {s}, Velocity (lookup): {v}, Distance to target (lookup): {d}")
        velocity, steering_angle = (self.max_velocity / 0.2) * v, math.degrees(s)
        
        self.get_logger().info(f"With change (scaled)::: Steering: {steering_angle}, Velocity: {velocity}")
        ...
        self.get_logger().info(f"Smoothed Velocity after S-curve: {self.current_velocity:.2f}")
        
        # Ensure the velocity is within safe limits based on the steering angle
        
        # Safe velocity (physics)
        safe_velocity_1 = self.calculate_safe_velocity(self.current_velocity, steering_angle)

        # # Extra curvature limit
        # k = 2.7
        # safe_velocity_2 = self.max_velocity / (1 + k * abs(self.calculate_curvature(1.5) ** 2))
        
        
        # To reduce wave curvature filter
        k = 3
        curv = abs(self.calculate_curvature(1))
        curv_threshold = 0.02
        min_motion_velocity = 0.15   # VERY IMPORTANT (tune 0.12–0.2)        
        if curv < curv_threshold:
            safe_velocity_2 = self.max_velocity
        else:
            safe_velocity_2 = max(min_motion_velocity, abs(self.max_velocity * np.exp(-k * (curv - curv_threshold))))
            # safe_velocity_2 = self.max_velocity / (1 + k * abs(self.calculate_curvature(1.5) ** 2))


        safe_velocity = min(abs(safe_velocity_1), abs(safe_velocity_2))

        self.get_logger().debug(f"safe_velocity_2: {safe_velocity_2:.2f}, safe_velocity_1: {safe_velocity_1:.2f}")
        self.get_logger().debug(f"safe_velocity: {safe_velocity:.2f}")
        self.current_velocity = min(abs(self.current_velocity), safe_velocity) * np.sign(self.current_velocity)
        self.current_velocity = min(abs(self.current_velocity), safe_velocity) * np.sign(self.current_velocity)
        
        self.get_logger().info(f"Current Velocity based on steering angle: {safe_velocity:.2f}")
        ...
        self.get_logger().info(f"Velocity: {self.current_velocity:.2f}, Wheel Vel: {self.wheel_velocity:.2f}")
        self.get_logger().info(f"Path Index: {self.path_index}")
        self.get_logger().info(f"Curvature: {self.calculate_curvature(1):.5f}")
        self.get_logger().info("-----")
    ...
    def calculate_safe_velocity(self, cv, steering_angle):
        ...
        self.get_logger().info(f"Curvature: {abs(curvature)}")
    ...
    def conflict_action_callback(self, msg):
        self.conflict_action = msg.data.strip().lower()
