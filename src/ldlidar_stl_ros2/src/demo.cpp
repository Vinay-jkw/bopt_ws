...
int main(int argc, char **argv) {
  ...
  node->declare_parameter<std::string>("serial_port", serial_port);
  ...
  node->declare_parameter<std::string>("communication_mode", communication_mode);
  // get ros2 param
  ...
  node->get_parameter("communication_mode", communication_mode);
  ...
  RCLCPP_INFO(node->get_logger(), "<serial_port>: %s", serial_port.c_str());
  RCLCPP_INFO(node->get_logger(), "<baud_rate>: %d", baud_rate);
  ...
  RCLCPP_INFO(node->get_logger(), "<communication_mode>: %s", communication_mode.c_str());
  ...
  ldlidarnode->EnableFilterAlgorithnmProcess(false); // default disable filter algorithm process, user can enable it according to actual demand
  bool is_started = false;
  
  if ( communication_mode == "COMM_UDP_MODE") {
     is_started = ldlidarnode->Start(type_name, server_ip.c_str(), server_port.c_str());
  } else if (communication_mode == "COMM_SERIAL_MODE") {
     is_started = ldlidarnode->Start(type_name, serial_port, baud_rate);
  } else {
    RCLCPP_ERROR(node->get_logger(), "Error, input <communication_mode> is illegal.");
    exit(EXIT_FAILURE);
  }
