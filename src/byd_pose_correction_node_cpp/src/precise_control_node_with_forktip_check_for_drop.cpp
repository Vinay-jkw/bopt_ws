...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr pallet_detected_sub;
...
public:
    ...
    void pallet_detected_callback(const std_msgs::msg::Bool::SharedPtr msg) {
        if (msg->data && distance < target_distance / 3.0) {  // Check pallet detection only for the first third of the distance
            pallet_detected = true;
            stop_due_to_pallet = true;
            RCLCPP_INFO(this->get_logger(), "Pallet detected within the first third of the journey.");
        }
    }
    ...
    void publish_messages() {
        if (pallet_detected || distance >= target_distance) {  // Update condition to check for distance limit
            timer->cancel();
            timer_alive = false;
            state = "Stop";
        ...
        // Adjusted velocity calculation for simplicity
        double target_velocity = (distance < target_distance / 2) ? max_velocity : min_velocity;
        velocity = (direction > 0) ? target_velocity : -target_velocity;
