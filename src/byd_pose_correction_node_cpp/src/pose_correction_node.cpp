  ...
  OrientRobot() : Node("orient_robot") {
    ...
    if (!this->get_parameter("target_z", z_param) ||
      ...
      RCLCPP_FATAL(this->get_logger(),
                   "Required parameter 'target_z' not provided.");
    ...
    if (!this->get_parameter("target_w", w_param) ||
      ...
      RCLCPP_FATAL(this->get_logger(),
                   "Required parameter 'target_w' not provided.");
    ...
    this->declare_parameter<double>("min_w", 0.15);
    ...
    this->declare_parameter<double>("pose_timeout_sec", 5.0);
    this->declare_parameter<int>("max_oscillations", 3);

    // ---- NEW: escape-from-loop parameters ----
    this->declare_parameter<double>("total_timeout_sec",
                                    15.0); // hard time bound
    this->declare_parameter<int>("max_tolerance_entries",
                                 3); // accept after N near-misses
...
private:
  ...
  void controlLoop() {
    ...
    if (elapsed > total_timeout_) {
      ...
      std::snprintf(
          buf, sizeof(buf),
          "Total timeout %.1fs reached. Accepting current orientation.",
          total_timeout_);
      stopAndShutdown(buf);
      return;
    }
    ...
    if (age > pose_timeout_) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                           "Pose is stale (%.2fs old). Holding last command "
                           "while waiting for AMCL update.",
                           age);
      // Do NOT publish a stop — that would kill relay priority and
      // prevent the steering interlock from completing. Instead keep
      // publishing the last commanded angular velocity so the steer
      // wheel continues aligning and /cmd_vel stays non-zero.
    ...
    if (abs_err > tolerance_) {
      ...
      if (last_error_sign_ != 0 && error * last_error_sign_ < 0) {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Overshoot detected (count=%d). Current err=%.4f",
                    oscillation_count_, error);
      ...
      last_cmd_ = cmd; // save for stale-pose replay
      ...
      if (in_tolerance_since_.nanoseconds() == 0) {
        ...
        if (tolerance_entries_ >= max_tol_entries_) {
          stopAndShutdown("Reached tolerance multiple times — accepting "
                          "current orientation.");
          return;
        }
      }
  ...
  geometry_msgs::msg::Twist last_cmd_; // replay during stale-pose window
};
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    RCLCPP_FATAL(rclcpp::get_logger("orient_robot"), "Startup failed: %s",
                 e.what());
    rclcpp::shutdown();
    return 1;
  }
...
//         z_param.get_type() == rclcpp::ParameterType::PARAMETER_NOT_SET)
//     {
//       RCLCPP_FATAL(this->get_logger(), "Required parameter 'target_z' not
//       provided."); throw std::runtime_error("Missing required parameter:
//       target_z");
//     }
//     if (!this->get_parameter("target_w", w_param) ||
//         w_param.get_type() == rclcpp::ParameterType::PARAMETER_NOT_SET)
//     {
//       RCLCPP_FATAL(this->get_logger(), "Required parameter 'target_w' not
//       provided."); throw std::runtime_error("Missing required parameter:
//       target_w");
//     }
...
//     if (std::abs(norm - 1.0) > 1e-3) {
//       RCLCPP_WARN(this->get_logger(),
//         "(target_z, target_w) not unit-norm (norm=%.4f). Normalizing.",
//         norm);
//       target_z_ /= norm;
//       target_w_ /= norm;
//     }
...
//     this->declare_parameter<double>("slowdown_zone", 0.785398);      // start
//     slowing ~45° out this->declare_parameter<double>("min_w", 0.08);
//     this->declare_parameter<double>("min_w_zone", 0.12);         // only
//     boost when error > this
//     this->declare_parameter<double>("hold_duration_sec", 0.3);
//     this->declare_parameter<double>("control_rate_hz", 20.0);
//     this->declare_parameter<double>("pose_timeout_sec", 2.0);
//     this->declare_parameter<int>("max_oscillations", 3);         // give up
//     after N overshoots
...
//                 "Target z=%.3f w=%.3f -> yaw=%.3f rad (%.1f deg). Control @
//                 %.1f Hz.", target_z_, target_w_, target_yaw_, target_yaw_ *
//                 180.0 / M_PI, rate_hz);
//     RCLCPP_INFO(this->get_logger(), "Waiting for /amcl_pose ...");
//   }
...
//   rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr cmd_pub_;
//   rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr
//   amcl_sub_; rclcpp::TimerBase::SharedPtr control_timer_;
