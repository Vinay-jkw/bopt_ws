...
class RosMqttBridge(Node):
    def __init__(self, params):
        ...
        self.remaining_path = []
        # ROS 2 Subscribers
        cp = self.create_subscription(PoseStamped, self.ros2_topics['current_pose'], self.pose_callback, qos_profile1)
        ...
        self.create_subscription(
            String,
            self.ros2_topics['path'],
            lambda msg: self.mqtt_client.publish(self.mqtt_topics['path'], msg.data, qos=2), 
            qos_profile
        )
        self.create_subscription(
            String,
            self.ros2_topics["holded_nodes"],
            lambda msg: self.mqtt_client.publish(self.mqtt_topics['holded_nodes'], msg.data),
            qos_profile
        )
        self.create_subscription(
            String,
        ...
        self.create_subscription(
            String,
            self.ros2_topics['safety_status'],
            lambda msg: self.mqtt_client.publish(self.mqtt_topics['safety_status'], json.dumps({"safety_status": msg.data})),
            10
        )
    ...
    def check_manager_heartbeat(self):
        ...
        if not alive and self.manager_alive:
            ...
            self.get_logger().warn("Manager heartbeat LOST!")
...
def main():
    ...
    robot_id = os.getenv('ROBOT_ID', 'BYD005')  # Default to 'BYD005' if not set
    defaults = {
        "mqtt_broker": "192.168.68.95",  # Or use: os.getenv('MQTT_BROKER')
        "mqtt_port": 1883,
        "reconnect_delay": 5,
        ...
        "mqtt_topics": {
            "path": f"{robot_id}/path",
            "task": f"{robot_id}/task",
            "holded_nodes": f"{robot_id}/holded_nodes",
            "conflict_action": f"{robot_id}/conflict_action",
            "task_action": f"{robot_id}/task_action",
            ...
            "safety_status": f"{robot_id}/safety_status",
